...
with $S_0(y_{other})\approx 1$. For the Gaia catalog selection function, the quantities $y_s$ are foremost: the sky position $(\alpha,\delta)$ of an object, its G-magnitude and its BP$_$RP color\footnote{For objects with very high proper motions, e.g. in the Solar system, the selection function may also depend on $\vec{\mu}$; this interesting regime, affecting a very small fraction of the Gaia detections, is not focus of this proposal.} These four quantities are a small subset of the Gaia catalog entries. 

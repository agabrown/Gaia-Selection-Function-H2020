...
\begin{figure}
    ...
    \caption{The completeness of the sample of sources at $12<G\leq15$ for which parallax and proper motion are listed Gaia DR2 and infrared photometry is listed in 2MASS. Note how combining these two catalogues leads to a drop in the completeness along the Galactic plane. This is because many sources in 2MASS will be invisible to Gaia due to the effects of dust. In addition in regions away from the Galactic plane artefacts related to the 2MASS sky survey are seen. This figure is an illustration of the selection function complexities that can arise when combing data from multiple catalogues.}
...
Unbiased distance estimates for all stars in samples of Gaia data (e.g. astrometry, RVS...) are also of vital interest to the community for studies of the dynamics of the Milky Way. Estimating the distances to stars in a Bayesian framework requires the Milky Way 3D map along with the selection function of the given sample. In it's simplest form, the distance PDF for any star in the sample will be given by
\begin{equation}
    \mathbb{P}(s | \varpi,l,b) = \frac{1}{Z} \mathbb{P}(\varpi | s, \sigma_\varpi) \mathbb{P}_\mathrm{map}(s, l, b) \mathbb{P}_\mathrm{obs}(S | s, l, b)
\end{equation}
where $\mathbb{P}_\mathrm{map}(s | l, b)$ refers to the 3D Milky Way map, $\mathbb{P}_\mathrm{obs}(S | s, l, b)$ is the selection function and $Z$ is a normalisation constant.
\\
...
\end{itemize}
And while better knowledge of the Gaia catalog selection function $S_C(y_s)$ is scientifically indispensable, it is not part of the DPAC deliverables.}
...
The objective of {\acro} is to derive a detailed description of the Gaia selection function which will be provided to and used by the scientific community. This work is divided into the tasks below which map to the work packages listed in \secref{sec:work-plan} (management of {\acro}, WP\ref{wp:management}, is discussed in \secref{sec:management}).

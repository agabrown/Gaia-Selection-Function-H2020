...
Given that $S_C(y_s)$ has not (yet) been determined for Gaia, why is it that many results of wide accepted results have already come from analysing the distribution of Gaia catalog entries, for large samples? This is for a number of reasons: first, some studies have essentially ignored (at their own peril) the selection function, thereby implicitly assuming $S_C(y_s)\approx 1$ for the regime of interest. The second regime is that one can make additional ``sample cuts'' by defining
\begin{equation*}
...
as the right hand side can be approximated as 
\begin{equation*}
